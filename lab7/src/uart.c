...
file_operations *init_dev_uart() { return &dev_file_operations; }

int dev_uart_write(file *file, const void *buf, size_t len) {
  // uart_log(DEBUG, "dev_uart_write\n");
  const char *char_buf = buf;
  for (int i = 0; i < len; i++)
    uart_putc(char_buf[i]);
  return len;
}

int dev_uart_read(file *file, void *buf, size_t len) {
  // uart_log(DEBUG, "dev_uart_read\n");
  char *char_buf = buf;
  for (int i = 0; i < len; i++)
    char_buf[i] = uart_getc();
  return len;
}

int dev_uart_open(vnode *file_node, file *target) {
  // uart_log(DEBUG, "dev_uart_open\n");
  target->vnode = file_node;
  target->f_ops = file_node->f_ops; //&dev_file_operations;
  return 0;
}

int dev_uart_close(file *file) {
  // uart_log(DEBUG, "dev_uart_close\n");
  kfree(file, SILENT);
  return 0;
}
