...
void irq_add_task(void (*callback)(), int order) {
  irqTask *task = kmalloc(sizeof(irqTask), SILENT);
  task->func = callback;
  task->order = order;
  task->busy = 0;
  task->prev = 0;
...
void irq_entry(trap_frame *tf) {
  ...
  if (*IRQ_PENDING_1 & (1 << 29)) { // UART interrupt
    ...
    case 0x4: // 0x4 UART Receive interrupt
      ...
      irq_add_task(uart_rx_irq_handler, ORDER_REGULAR);
      break;
    }
    ...
    irq_add_task(timer_irq_handler, ORDER_FIRST);
  }
  ...
  while (irqTask_head != 0 && !irqTask_head->busy) {
    ...
    if (task == irqTask_head)
      irqTask_head = task->next;
    kfree((void *)task, SILENT);
    enable_interrupt();
  }
  do_signal(tf);
}