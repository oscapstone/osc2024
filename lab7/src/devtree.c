...
void fdt_traverse(void (*callback)(void *, char *)) {
  ...
  DTB_BASE = (void *)TO_VIRT(DTB_BASE);
  DTB_END = DTB_BASE + be2le(&header->totalsize);

  uart_log(INFO, "Dtb loaded at ");
  uart_addr_range((uintptr_t)DTB_BASE, (uintptr_t)DTB_END);
  uart_putc(NEWLINE);
  ...
  while (ptr < structure + structure_size) {
    ...
    switch (token) {
      ...
      case FDT_END:
        break;
      default:
        uart_log(WARN, "Unknown fdt token: ");
        uart_simple_hex(token);
        uart_putc(NEWLINE);
    }
  }
}