...
static inline size_t get_node_name(char *c_filepath, size_t start_index, size_t *end_index, char buf[])
{
	...
	*end_index = i;
	...
	{
		uart_puts("get_node_name: unknown type\r\n");
		return -1;
	}
	uart_puts("get_node_name: %s, type: %d, i: %d, c_filepath[i]: %c\r\n", buf, type, i, c_filepath[i]);
	return type;
}
...
static inline void __gen_vnode_of_initramfs(mount_t *_mount)
{
	...
	CPIO_FOR_EACH(&c_filepath, &c_filesize, &c_filedata, error, {
		...
		while (end)
		{
			...
			uart_puts("initramfs_setup_mount: c_filepath: %s, compoent_name: %s, strlen: %d, type: %d\r\n", c_filepath, compoent_name, strlen(compoent_name), type);
...
initramfs_inode_t *initramfs_create_inode(enum fsnode_type type, const char *name, char *data, size_t datasize)
{
	...
	uart_puts("initramfs_create_inode: %s\r\n", name);
	strcpy(inode->name, name);
	uart_puts("initramfs_create_inode: inode->name %s\r\n", inode->name);
	if (type == FS_DIR)
	{
	...
	{
		uart_puts("initramfs_create_inode: unknown type\r\n");
		return NULL;
	}
	return inode;
}
...
{
	uart_puts("initramfs can't call write\r\n");
	return -1;
}
...
int __initramfs_mkdir(struct vnode *dir_node, struct vnode **target, const char *component_name)
{
	...
	{
		uart_puts("initramfs mkdir not dir_t\r\n");
		return -1;
	}
...
{
	struct initramfs_inode *inode = dir_node->internal;
	// uart_puts("initramfs_readdir: %s\r\n", dir_node->name);

	if (dir_node->type != FS_DIR)
	{
		uart_puts("initramfs readdir not dir_t\r\n");
		return -1;
	}
	...
	{
		child_vnode = ((vnode_list_t *)curr)->vnode;
		// uart_puts("initramfs_readdir: child_vnode->name %s\r\n", child_vnode->name);
