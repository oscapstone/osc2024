...
// Basic 4
// 創建並註冊了一個名為 "initramfs" 的file system
// 並指定其mount設置initramfs_setup_mount
int register_initramfs()
{
    struct filesystem fs;
    // 創一個filesystem structure "initramfs"
    fs.name = "initramfs";
    // 將"initramfs" setup_mount設為`initramfs_setup_mount` function
    fs.setup_mount = initramfs_setup_mount;
    // 通過register_filesystem function將其註冊到VFS中
    return register_filesystem(&fs);
}

// 設置 initramfs file system的掛載點
int initramfs_setup_mount(struct filesystem *fs, struct mount *_mount){
    ...
    {
        // 將 CPIO的file system內容解析並加載到memory中
        ...
        if (header_pointer != 0)
        {
            ...
            // 將每個file的 vnode 加到root dir的entry中
...
struct vnode *initramfs_create_vnode(struct mount *_mount, enum fsnode_type type){
    // 設置其file和vnode的operation，並分配memory給其 inode
...
// file operations
// write operation return false，因為 initramfs是read-only
