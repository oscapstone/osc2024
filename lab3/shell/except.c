...
void irq_except_handler() {
    ...
    // 設定timeout後，當發生interrupt時，timeout interrupt handler處理interrupt
    ...
    // task interrupt
    // CNTPSIRQ : Counter Physical Secure IRQ" status.
    // 如果在secure mode下運作並且想要handle timer interrupt
    // 使用CNTPSIRQ因為系統可能在non-secure mode
