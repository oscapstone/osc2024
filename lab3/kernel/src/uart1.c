...
#include "exception.h"
#include "u_string.h"
// VSPRINT_MAX_BUF_SIZE=0x100=256
...
char uart_async_getc() {
    *AUX_MU_IER_REG |=1; // enable read interrupt
    ...
    while (uart_rx_buffer_ridx == uart_rx_buffer_widx) *AUX_MU_IER_REG |=1; // 當read buffer的idx＝＝write buffer的idx時，代表buffer為空
     // enable read interrupt
    el1_interrupt_disable();
    char r = uart_rx_buffer[uart_rx_buffer_ridx++];
    if (uart_rx_buffer_ridx >= VSPRINT_MAX_BUF_SIZE) uart_rx_buffer_ridx = 0;// 當read buffer的idx大於原本設定的max_size時，重設read buffer idx為0
                                                                             // 實現circular buffer // uart_rx_buffer_ridx設為0只是為了讓之後再呼叫這個function時可以繼續正常使用（只是用完歸零的概念）
...
void uart_async_putc(char c) {
    ...
    el1_interrupt_enable();
    *AUX_MU_IER_REG |=2;  // enable write interrupt // 此時觸發interrupt->接下來cpu會調用exception vector去處理interrupt
}
...
void uart_interrupt_enable(){
    ...
    *ENABLE_IRQS_1  |= 1 << 29;    // Pg.112 // Pg.113: ARM peripherals interrupts table.
}
