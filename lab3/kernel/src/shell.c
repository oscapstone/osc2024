...
void do_cmd_exec(char* filepath)
{
    ...
    while(header_ptr!=0)
    {
        ...
        if(strcmp(c_filepath, filepath)==0)
        {
            ...
            asm("msr elr_el1, %0\n\t"   // elr_el1: Set the return address: c_filedata
                "msr spsr_el1, xzr\n\t" // spsr_el1[9:6]: enable interrupt. spsr_el1[3:0]: EL0t
                "msr sp_el0, %1\n\t"    // user program stack pointer set to new stack
                "eret\n\t"              // Perform exception return. EL1 -> EL0
                :: "r" (c_filedata),
                   "r" (ustack+USTACK_SIZE)); // stack grow from high address to low address
            free(ustack);
            break;
        }
