...
#define USTACK_SIZE 0x10000 //=65536 bytes
...
void do_cmd_exec(char* filepath)
{
    ...
    while(header_ptr!=0)
    {
        ...
        {
            // exec c_filedata
            // 印出c_filedata的address
            
            char* ustack = kmalloc(USTACK_SIZE); //malloc 65536 bytes 
            /*
            * msr elr_el1<-x0
            * msr spsr_el1<-x1
            * msr sp_el0<-x1
            */ 
            // uart_puts("user_proc.img address: 0x%x\n", c_filedata);
            asm("msr elr_el1, %0\n\t"   // elr_el1: Set the address to return to: c_filedata
                "mov x1, 0x3c0\n\t"
                "msr spsr_el1, x1\n\t" // enable interrupt (PSTATE.DAIF) -> spsr_el1[9:6]=4b0. In Basic#1 sample, EL1 interrupt is disabled. // xzr: Zero register
...
{
    uart_puts("dtb address: 0x%x\r\n", dtb_ptr);
    uart_puts("CPIO address: 0x%x\r\n", CPIO_DEFAULT_PLACE);
    char* c_filepath;
    char* c_filedata;
    unsigned int c_filesize;
    struct cpio_newc_header *header_ptr = CPIO_DEFAULT_PLACE;
...
{
    add_timer(uart_sendline,atoi(sec),msg); //char to long long:輸入char會轉成long long
}
