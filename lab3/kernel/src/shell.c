...
void do_cmd_exec(char* filepath)
{
    ...
    while(header_ptr!=0)
    {
        ...
        if(strcmp(c_filepath, filepath)==0) 
        {
            // kernel轉移給user program EL1 -> EL0
            // load program in EL1 and exec c_filedata program in EL0
            char* ustack = kmalloc(USTACK_SIZE); // 分配新的st給program
            // asm: 直接向CPU registert寫入數據
