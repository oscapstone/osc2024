...
void core_timer_handler() {
  if (list_empty(timer_event_list)) {
    ...
    unsigned long long absent_time = get_tick_plus_s(10000);
...
void set_core_timer_interrupt(unsigned long long expired_time) {
  __asm__ __volatile__(
      ...
      "mul x1, x1, %0\n\t" // cntpct_el0 = cntfrq_el0 * seconds: relative timer
                           // to cntfrq_el0
      "msr cntp_tval_el0, x1\n\t" // Set expired time to cntp_tval_el0, which
                                  // stores time value of EL1 physical timer.
      : "=r"(expired_time));
}
