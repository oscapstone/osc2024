...
void el1h_irq_router() {
  ...
  // Pg.113 (2)
  // https://datasheets.raspberrypi.com/bcm2836/bcm2836-peripherals.pdf - Pg.16
  if (*IRQ_PENDING_1 & IRQ_PENDING_1_AUX_INT &&
      *CORE0_INTERRUPT_SOURCE &
          INTERRUPT_SOURCE_GPU) // from aux && from GPU0 -> uart exception
  {
    if (*AUX_MU_IIR_REG & (2 << 1)) // Interrupt ID bits
    {
      //uart_sendline("A");
    ...
    else if (*AUX_MU_IIR_REG & (1 << 1)) // FIFO clear bits
    {
      //uart_sendline("B");
      *AUX_MU_IER_REG &= ~(2); // disable write interrupt
      irqtask_add(uart_w_irq_handler, UART_IRQ_PRIORITY);
      irqtask_run_preemptive(); // run the queued task before returning to the
                                // program.
    }
  } else if (*CORE0_INTERRUPT_SOURCE &
             INTERRUPT_SOURCE_CNTPNSIRQ) // from CNTPNS (core_timer) // A1 -
                                         // setTimeout run in el1
  {
    core_timer_disable();
    irqtask_add(core_timer_handler, priority--);
    irqtask_run_preemptive();
    core_timer_enable();
  }
}
...
void el0_sync_router() {
  ...
  __asm__ __volatile__(
      "mrs %0, SPSR_EL1\n\t"
      : "=r"(spsr_el1)); // EL1 configuration, spsr_el1[9:6]=4b0 to enable
                         // interrupt
  ...
  __asm__ __volatile__(
      "mrs %0, ESR_EL1\n\t"
      : "=r"(esr_el1)); // ESR_EL1 holds symdrome information of exception, to
                        // know  the cause od that exception
  uart_sendline(
      "[Exception][el0_sync] spsr_el1 : 0x%x, elr_el1 : 0x%x, esr_el1 : 0x%x\n",
...
/*
Preemption
Now, any interrupt handler can preempt the task’s execution, but the newly
enqueued task still needs to wait for the currently running task’s completion.
It’d be better if the newly enqueued task with a higher priority can preempt the
currently running task. To achieve the preemption, the kernel can check the last
executing task’s priority before returning to the previous interrupt handler. If
there are higher priority tasks, execute the highest priority task.
*/
...
void irqtask_add(void *task_function, unsigned long long priority) {
  irqtask_t *the_task =
      kmalloc(sizeof(irqtask_t)); // free by irq_tasl_run_preemptive()
...
void irqtask_run_preemptive() {
  ...
  while (!list_empty(task_list)) {
    ...
    // Run new task (early return) if its priority is lower than the scheduled
    // task.
    ...
    // the kernel can check the last executing task’s priority before returning
