...
void str_int_2_decimal(char *buf, int value) {
  str_num_2_decimal(buf, value < 0 ? -value : value, value < 0);
}

void str_uint_2_decimal(char *buf, unsigned int value) {
  str_num_2_decimal(buf, value, 0);
}
...
void vsnprintf(char *buf, unsigned int size, const char *fmt,
  ...
  for (p = fmt; *p != '\0' && out < buf_end; ++p) {
    ...
    case 'x':
      u = __builtin_va_arg(args, unsigned int);
      str_ulong_2_hex(temp, (unsigned long)u);
      for (s = temp; *s && out < buf_end; ++s) {
        *out++ = *s;
      }
      break;
