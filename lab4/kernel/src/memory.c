...
int memory_reserve(void *start, void *end)
{
    lock();
    size_t start_index = (size_t)start / PAGE_FRAME_SIZE;                                            // align
    size_t end_index = (size_t)end / PAGE_FRAME_SIZE + ((size_t)end % PAGE_FRAME_SIZE == 0 ? 0 : 1); // padding
    // split the start frame to fit the start address
    char start_max_val = 0;
    frame_t *start_frame = index_to_frame(start_index);
    frame_t *end_frame = index_to_frame(end_index);
    frame_t *start_split_frame = get_buddy_frame(start_max_val, start_split_frame);
    frame_t *iterator_frame = start_frame;
    ...
    // while (iterator_frame->val == F_FRAME_VAL)
    // {
    //     iterator_frame--;
    // }
    ...
    while (curr_index <= end_index)
    {
        ...
        {
            uart_puts("curr_index is over end_index, curr_index: (0x%x, 0x%x), end_index: (0x%x, 0x%x)\n", curr_index, frame_addr_to_phy_addr(curr_frame), end_index, frame_addr_to_phy_addr(end_frame));
            size_t split_val = curr_frame->val - 1;
            size_t buddy_index = get_buddy_index(split_val, curr_index);
            frame_t *buddy_frame = index_to_frame(buddy_index);
            curr_frame->val = split_val;
    ...
    // while (iterator_frame <= end_frame)
    // {
    //     if (iterator_frame < start_frame)
    //     {
    //         uart_puts("iterator_frame is before start_frame, iterator_frame: (0x%x, 0x%x), start_frame: (0x%x, 0x%x)\n", iterator_frame, frame_addr_to_phy_addr(iterator_frame), start_frame, frame_addr_to_phy_addr(start_frame));
    //         frame_t *buddy = get_buddy_frame(iterator_frame->val - 1, iterator_frame);
    //         uart_puts("iterator_frame: (0x%x, 0x%x), iterator_frame->val: %d, buddy: (0x%x, 0x%x), buddy->val: %d\n", iterator_frame, frame_addr_to_phy_addr(iterator_frame), iterator_frame->val, buddy, frame_addr_to_phy_addr(buddy), buddy->val);
    //         buddy->val = iterator_frame->val - 1;
    //         iterator_frame->val--;
    //         page_insert(buddy->val, buddy);
    //         list_del_entry(iterator_frame);
    //         page_insert(iterator_frame->val, iterator_frame);
    //         if (buddy <= start_frame)
    //         {
    //             uart_puts("iterator_frame = buddy, buddy: (0x%x, 0x%x), buddy->val: %d\n", buddy, frame_addr_to_phy_addr(buddy), buddy->val);
    //             iterator_frame = buddy;
    //         }
    //     }
    //     else if (start_frame <= iterator_frame && iterator_frame + val_to_num_of_frame(iterator_frame->val) - 1 <= end_frame)
    //     {
    //         uart_puts("reserve frame: (0x%x -> 0x%x), iterator_frame->val: %d\n", frame_addr_to_phy_addr(iterator_frame), frame_addr_to_phy_addr(iterator_frame + val_to_num_of_frame(iterator_frame->val) - 1), iterator_frame->val);
    //         list_del_entry(iterator_frame);
    //         iterator_frame += val_to_num_of_frame(iterator_frame->val);
    //     }
    //     else if (start_frame < iterator_frame && end_frame < iterator_frame + val_to_num_of_frame(iterator_frame->val) - 1)
    //     {
    //         uart_puts("iterator_frame is over end_frame, iterator_frame: (0x%x, 0x%x), end_frame: (0x%x, 0x%x)\n", iterator_frame, frame_addr_to_phy_addr(iterator_frame), end_frame, frame_addr_to_phy_addr(end_frame));
    //         frame_t *buddy = get_buddy_frame(iterator_frame->val - 1, iterator_frame);
    //         buddy->val = iterator_frame->val - 1;
    //         iterator_frame->val--;
    //         page_insert(buddy->val, buddy);
    ...
    //     }
    // }
    // uart_puts("end reserve: iterator_frame: (0x%x, 0x%x), end_frame: (0x%x, 0x%x)\n", iterator_frame, frame_addr_to_phy_addr(iterator_frame), end_frame, frame_addr_to_phy_addr(end_frame));
    uart_puts("end reserve: curr_index: (0x%x, 0x%x), end_index: (0x%x, 0x%x)\n", curr_index, frame_addr_to_phy_addr(index_to_frame(curr_index)), end_index, frame_addr_to_phy_addr(end_frame));
    unlock();
    return 0;
}
...
#define TEST_NUM 4
    ...
    // dump_frame();
    // int size[TEST_NUM] = {5, 20, 430, 3000, 50, 50, 50, 50};
    // int malloc_order[TEST_NUM] = {0, 3, 2, 1, 4, 5, 6, 7};
    // void *ptr[TEST_NUM];
    // int free_order[TEST_NUM] = {1, 3, 2, 0, 4, 5, 6, 7};
    // dump_cache();
    // for (size_t i = 0; i < TEST_NUM; i++)
    // {
    ...
    // }
    // uart_puts("END\r\n");
    memory_reserve((void *)0x10000, (void *)0x20000);
    memory_reserve((void *)0x80000, (void *)0x1032000);
    dump_frame();
    while (1)
        ;
}
