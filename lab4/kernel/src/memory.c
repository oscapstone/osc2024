...
unsigned int getidx(frame_t *frame)
{
}
...
void *page_malloc(unsigned int size)
{
    ...
    // uart_sendlinek("target_frame_ptr idx :%d\n", target_frame_ptr->idx);
    // uart_sendlinek("buddyptr idx :%d\n", get_buddy(target_frame_ptr)->idx);
    // uart_sendlinek("buddyptr used :%d\n", get_buddy(target_frame_ptr)->used);
...
void page2caches(int c_val)
{
    ...
    // int num = (PAGESIZE >> (c_val + CACHE_offset));
    // cache_t *cache_array = allocator(num * sizeof(cache_t));
    ...
    // uart_sendlinek("cache_order :%d\n", target_frame_ptr->cache_order);
    // uart_sendlinek("val :%d\n", target_frame_ptr->val);
    // uart_sendlinek("used :%d\n", target_frame_ptr->used);

    // for (int i = 0; i < num; i++)
    // {
    //     cache_array[i].pos = i * (1 << (c_val + CACHE_offset)) + ptr;
    //     list_add(&(cache_array[i].listhead), &cache_list[c_val]);
    // };
    list_add(&(Pageinfo->listhead), &cache_list[c_val]);
    // return Pageinfo;
}

// recursion method to find free page
// frame_t *find_free_page(int val)
// {
//     frame_t *target_frame_ptr = (frame_t *)frame_freelist[val].next;
//     if (!list_empty(&frame_freelist[val]))
//     {
//         list_del_entry((struct list_head *)target_frame_ptr);
//         return target_frame_ptr;
//     }

//     target_frame_ptr = find_free_page(val + 1);
...
void cache_free(void *ptr)
{
    ...
    cache_t *cache_ptr = idx * PAGESIZE;
    ...
    // int c_val = target_frame_ptr->cache_order;
    // cache_t *cache = allocator(sizeof(cache_t));
    // cache->pos = ptr;
