...
static char *uart_read_buffer;
static char *uart_write_buffer;

void init_uart() {
  uart_puts("[INFO] initializing uart_read_buffer.");
  uart_putc(NEWLINE);
  uart_read_buffer = (char *)kmalloc(UART_BUF_SIZE, 0);

  uart_puts("[INFO] initializing uart_write_buffer.");
  uart_putc(NEWLINE);
  uart_write_buffer = (char *)kmalloc(UART_BUF_SIZE, 0);
