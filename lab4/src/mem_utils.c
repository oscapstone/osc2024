...
void buddy_system_init(void)
{
    ...
    /* reserve area */
    memory_reserve((char *)0x0000, (char *)0x1000);                     // spin tables
    memory_reserve(cpio_addr, cpio_addr_end);                           // initramfs
    memory_reserve(dtb_start, dtb_end);                                 // device tree
    memory_reserve(kernel_start, kernel_end);                           // kernel
    memory_reserve((char *)0x80000 - 2 * PAGE_SIZE, (char *)0x80000);   // sp
