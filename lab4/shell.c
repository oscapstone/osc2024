...
void shell_run() {
  ...
  while (1) {
    ...
    if (strcmp(token, "help") == 0) {
      ...
      // } else {
      //   uart_sendline("Usage: exec <program_name>\n");
      // }
      ...
      buddy_system_print_freelists(atoi(show_bitmap));
      ...
      if (addr == 0) {
        uart_sendline("Invalid address!\n");
        continue;
      }
      buddy_system_free(addr);
      buddy_system_print_freelists(0);
...
void do_cmd_help() {
  ...
  format_command("freelist [show_bitmap]",
                 "Show free block lists for each level of the buddy system.");
  format_command("malloc <size>",
...
void do_cmd_malloc(unsigned int size) {
  if (size == 0 || size > (1 << MAX_LEVEL) * PAGE_SIZE) {
    uart_sendline("Invalid size!\n");
    return;
  }
  if (size <= 1024) {
    memory_pool_allocator(size);
  } else {
    buddy_system_allocator(size);
  }
}