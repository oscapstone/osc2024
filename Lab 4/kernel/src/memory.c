...
kmemory_alloc(uint32_t size)
{
    ...
    uint32_t count = UINT32_PADDING((size + BLOCK_HEADER_SIZE), FRAME_SIZE_ORDER) >> FRAME_SIZE_ORDER;
    ...
    block[1] = (uint8_t) size + BLOCK_HEADER_SIZE;
