...
void parse_command(char* buffer){
	...
	if(utils_string_compare(input_string,"help")) {
	   ...
	   uart_send_string("timer	:set 3 timer at once\n");
	   uart_send_string("settimeout	:set a timeout\n");
       ...
       uart_send_string("Hello World!\n");
		 ...
		 unsigned int *a = simple_malloc(4);
		 uart_hex(a);
		 uart_send_char('\n');
		...
		char seconds[BUFFER_MAX_SIZE];
		read_command(seconds);
		size_t seconds_size = utils_strlen(seconds) - 1;
		uint64_t time_sec = utils_atoi(seconds,seconds_size);
		//size_t second_str_len = utils_strlen(parameter[1]);
		//uint64_t seconds = (uint64_t) utils_atoi(parameter[1],second_str_len-1);
		setTimeout(message,time_sec);
		uart_send_string("\n");
	 }	else {
