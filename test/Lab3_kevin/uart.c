...
#define RX_INTERRUPT_BIT    0x01    //=> 0001
#define TX_INTERRUPT_BIT    0x02    //=> 0010
...
void uart_num(unsigned long long d) {
    unsigned long long n;
    int c;
    for(c=28;c>=0;c-=4) {
        // get highest tetrad
...
void uart_enable_interrupt() {
	...
	uint32_t ier = mmio_read(AUX_MU_IER);   //p.12 AUX_MU_IER[0] = 1 Enable receive interrupts.
    ier |= RX_INTERRUPT_BIT;    //If this bit is set the interrupt line is asserted whenever the receive FIFO holds at least 1 byte.
    ...
    uint32_t enable_irqs1 = (uint32_t) ENABLE_IRQS_1;   //p.116
    enable_irqs1 |= AUXINIT_BIT_POSTION; // Set bit29   p.113
