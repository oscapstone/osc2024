...
void init_mem() {
  ...
  uart_log(INFO, "Simple initialized.\n");
  ...
  unsigned int page_table = NUM_PAGES * sizeof(struct page) * 2;

  // Reserve memory:
  // Spin tables for multicore boot
  reserveMemory(0x0, 0x1000);
  // Kernel & stack; Simple allocator
  reserveMemory(0x80000 - STACK_SIZE, (unsigned long)&__bss_end + page_table);
  // Initramfs
