...
void timer_irq_handler() {
  // Set up the next timer interrupt (frequency >> 5, aka. 1/32 of the
  // frequency)
  asm volatile(
      "mrs x0, cntfrq_el0\n"
      "lsr x0, x0, 5\n"
  ...
  while (head != 0 && timer_get_uptime() >= head->time) {
    ...
    kfree(head, 0);         // Free the memory allocated for the timer node
...
void timer_add(void (*callback)(void *), void *arg, int duration) {
  ...
  timerEntry *timer = kmalloc(sizeof(timerEntry), 1);
  timer->func = callback;
  timer->arg = arg;
...
static void show_timer_message(void *arg) {
  ...
  uart_puts((char *)arg);
  uart_putc(NEWLINE);
  kfree(arg, 1);  // Free the memory allocated for arg
}
