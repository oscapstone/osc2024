...
{
    uart_sendlinek("in syscall exec\n");
    curr_thread->datasize = get_file_size((char *)name);
    char *new_data = get_file_start((char *)name);
    // uart_sendlinek("file_size : %d \n",get_file_size((char *)name));
...
int fork(trapframe_t *tpf)
{
    ...
    // uart_sendlinek("fork _ curr_thread->context.lr: %x\n",curr_thread->context.lr);
    ...
    unlock();
    tpf->x0 = newt->pid; // pid = new
    // schedule();
    return newt->pid;

child:
    // the offset of current syscall should also be updated to new return point
    // uart_sendlinek("child elr_el1 : %x\n",tpf->elr_el1);
    // uart_sendlinek("curr_thread->data : %x\n",(unsigned long)(curr_thread->data));
    // uart_sendlinek("curr_thread->datasize : %x\n",curr_thread->datasize);
