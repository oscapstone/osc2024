...
int get_arm_memory_info(unsigned int *base_address, unsigned int *size)
{
  ...
  do_mbox_call(MBOX_CH_PROP); // message passing procedure call, you should implement it following the 6 steps provided above.
...
int mbox_call(mbox_channel_type channel, unsigned int value)
{
  ...
  *MBOX_WRITE = r;
  ...
  unlock();
  // return ((unsigned int *)value)[1] == MBOX_REQUEST_FAILED;
}