...
int mbox_call( mbox_channel_type channel, unsigned int value )
{
    ...
    value &= ~(0xF);
    value |= channel & 0xF; // Ensure only lower 4 bits are set
    while ( (*MBOX_STATUS & BCM_ARM_VC_MS_FULL) != 0 ) {} // Wait until mailbox is not full
    // Write to Register
    *MBOX_WRITE = value;
    while(1) {
        while ( *MBOX_STATUS & BCM_ARM_VC_MS_EMPTY ) {}// Wait until mailbox is not empty
        // Read from Register
        if (value == *MBOX_READ)
