...
// AUX_MU_IER_REG the controler for enable/disable {receive/transmit}

// if setted
// bit 1 enable transmit
// bit 0 enable  receive
...
// AUX_MU_IO_REG -> Read data to ot Write data from register
    // In this register, it only care about least 8 bit
        // Hence only one byte
...
void uart_handler(void *arg)
{
    ...
    {
        // AUX_MU_LSR_REG -> Bit5 : This bit is set if the transmit FIFO can accept at least one byte
        while (*AUX_MU_LSR_REG & 0x20)
        {
