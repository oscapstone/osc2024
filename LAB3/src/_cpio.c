...
void cpio_load_program(char *filename)
{
    ...
    if (prog_addr)
    {
        ...
        unsigned char *target = (unsigned char *)user_addr;
        while (file_size--)
        {
            *target = *file_content;
            target++;
        ...
        // bit 7 is the irq mask bit 
            // if mask => no irq handling
        asm volatile("mov x0, 0x340  \n");
        asm volatile("msr spsr_el1, x0   \n");
        asm volatile("msr elr_el1, %0    \n" ::"r"(user_addr));
        asm volatile("msr sp_el0, %0    \n" ::"r"(user_addr + USER_STACK_SIZE));
        asm volatile("eret   \n");
        // asm volatile
        // (
    ...
    {
        uart_send_string("No Such Program\n");
    }
}
