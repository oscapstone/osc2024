...
void fdt_traverse( void (*callback)(char *, char *, struct fdt_prop *) ){
    ...
    if (!strncmp((char*)DEVTREE_ADDRESS, magic, 4)){
        print_newline();
        print_hex(*(uint32_t*)DEVTREE_ADDRESS);
        print_str("\nMagic Fail");
        return;
    ...
    void *dt_struct_addr = DEVTREE_ADDRESS + to_little_endian(devtree_header->off_dt_struct);
    char *dt_string_addr = DEVTREE_ADDRESS + to_little_endian(devtree_header->off_dt_strings);
