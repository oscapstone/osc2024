...
uintptr_t get_dtb_start(void) { return _dtb_start; }

void set_dtb_start(uintptr_t ptr) { _dtb_start = ptr; }

uintptr_t get_dtb_end(void) { return _dtb_end; }

void set_dtb_end(uintptr_t ptr) { _dtb_end = ptr; }
...
uint32_t fdt_traverse(fdt_callback cb) { return fdt_struct_parse(cb); }
...
void fdt_find_root_node(uint32_t token, const char *name, const void *data,
                        uint32_t UNUSED(size)) {
  switch (token) {
  case FDT_BEGIN_NODE:
    if (!str_cmp(name, ""))
...
uintptr_t get_usable_mem_start(void) { return usable_start_addr; }

uintptr_t get_usable_mem_length(void) { return usable_length; }

void fdt_find_memory_node(uint32_t token, const char *name, const void *data,
                          uint32_t UNUSED(size)) {
  switch (token) {
  case FDT_BEGIN_NODE:
