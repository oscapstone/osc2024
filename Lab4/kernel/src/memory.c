...
static uintptr_t usable_mem_start;  // 0x0
static uintptr_t usable_mem_length; // 0x3C000000
static uintptr_t usable_mem_end;    // 0x3C000000

static uintptr_t cpio_start; // 0x8000000
static uintptr_t cpio_end;   // 0x8000A00

static uintptr_t dtb_start; // 0x8200000
static uintptr_t dtb_end;   // 0x8215988
...
int mem_init(uintptr_t dtb_ptr) {
  ...
  // node), and use this information to extract the reg property of memory node.
 ...
 */
static struct list_head **buckets;
...
uint8_t *alloc_pages(size_t request) {
   ...
   */
  // since the type of bucket is of size_t (aka unsigned long long), we have to
  ...
  while (bucket + 1 != 0) {
    ...
    if (!ptr) {
#if defined(DEBUG)
      uart_printf("Free list for order %d is empty. "
...
#endif
      list_push(buckets[bucket], (struct list_head *)ptr);
      return NULL;
    }
...
#if defined(DEBUG)
      uart_printf("Splitting order %d page into half: 0x%x - "
                  "0x%x",
                  get_order_from_bucket(bucket),
...
void free_pages(uint8_t *ptr) {
  ...
  if (merged_addr < usable_mem_end)
    list_push(buckets[bucket], (struct list_head *)merged_addr);
}

/*
 * register the reserve region, it needs to be called before
 * `start_init_pages` function.
 */
static void register_reserve_pages(uintptr_t start, uintptr_t end) {
#if defined(DEBUG)
  uart_printf("register\n");
#endif
  ...
  start = (uintptr_t)PAGE_ALIGN_DOWN(start - (uintptr_t)base_ptr) +
      ...
      (uintptr_t)PAGE_ALIGN_UP(end - (uintptr_t)base_ptr) + (uintptr_t)base_ptr;
  ...
  size_t end_idx = get_index_from_ptr(end, MAX_ALLOC_LOG2);
...
static void start_init_pages(void) {
  ...
  /*
   * if there is no page need to be reserved, push the whole memory region onto
   * the free list with maximum order
   */
  if (!reserve_flag) {
    list_push(buckets[0], (struct list_head *)base_ptr);
    return;
  }

  // FIXME: Maybe get set bit of node_is_reserved for each iteration.
...
#if defined(DEBUG)
      uart_printf("found first split, index: 0x%x, order: %d, addr: 0x%x\n",
...
#endif
      list_push(buckets[curr_bucket], (struct list_head *)get_ptr_from_index(
                                          first_split, curr_bucket));
#if defined(DEBUG)
...
void buddy_init(void) {
...
#if defined(DEBUG)
  uart_printf("usable_mem_start: 0x%x\n", usable_mem_start);
  uart_printf("usable_mem_end: 0x%x\n", usable_mem_end);
  ...
  for (int i = 0; i < BUCKET_COUNT; i++) {
    buckets[i] = (struct list_head *)mem_alloc(sizeof(struct list_head));
  ...
  register_reserve_pages(usable_mem_end, usable_mem_start + MAX_ALLOC);
...
void print_free_list(void) {
  ...
  for (int i = MIN_ALLOC_LOG2; i < MIN_ALLOC_LOG2 + BUCKET_COUNT; i++) {
    ...
    uart_printf("HEAD(0x%x) -> ", &buckets[i]);
    struct list_head *node;
    list_for_each(node, buckets[i]) { uart_printf("0x%x -> "); }
    uart_printf("\n");
  }
}
