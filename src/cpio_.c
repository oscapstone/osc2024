...
void cpio_ls() {
    char *addr = cpio_addr;
    while (strcmp((char *)(addr + sizeof(struct cpio_newc_header)), "TRAILER!!!") != 0) {
        cpio_header *header = (cpio_header *)addr;
        unsigned int filename_size = atoi(header->c_namesize, (int)sizeof(header->c_namesize));
        unsigned int headerPathname_size = sizeof(cpio_header) + filename_size;
        unsigned int file_size = atoi(header->c_filesize, (int)sizeof(header->c_filesize));
...
char *findFile(const char *name) {
    ...
    while (strcmp((char *)(addr + sizeof(struct cpio_newc_header)), "TRAILER!!!") != 0) {
        ...
        cpio_header *header = (cpio_header *)addr;
        unsigned int pathname_size = atoi(header->c_namesize, (int)sizeof(header->c_namesize));
        unsigned int file_size = atoi(header->c_filesize, (int)sizeof(header->c_filesize));
