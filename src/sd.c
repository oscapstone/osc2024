...
void print_mbr_info(void) {
    ...
    mbr = (struct general_mbr *)buf;

    printf("\nmbr boot signature: %d\n", mbr->boot_signature == MBR_BOOT_SIG);

    partition_entry = (char *)mbr->partitions;
    printf("partition entry: ");
    for (int i = 0; i < 16; i++) {
        printf("%2x ", partition_entry[i]);
    }

    printf("\npartition boot indicator: %x\n", mbr->partitions[0].boot_indicator);
    printf("starting sector 0x%x = %d\n", mbr->partitions[0].starting_sector, mbr->partitions[0].starting_sector);
    printf("partition size 0x%x = %d\n", mbr->partitions[0].partition_size, mbr->partitions[0].partition_size);
}
