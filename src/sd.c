...
#define INT_ERROR_MASK      0x017E8000 // 0b0000 0001 0111 1110 1000 0000 0000 0000. bits 15: error occurs.
...
{
    // wait up to 500ms for status
    int cnt = 500000; while((EMMC->STATUS & mask) && !(EMMC->INTERRUPT & INT_ERROR_MASK) && cnt--) wait_msec(1);
...
int sd_int(unsigned int mask)
{
    ...
    int cnt = 1000000; while(!(EMMC->INTERRUPT & m) && cnt--) wait_msec(1);
    r=EMMC->INTERRUPT;
...
int sd_readblock(unsigned int lba, unsigned char *buffer, unsigned int num)
{
    ...
    if(sd_scr[0] & SCR_SUPP_CCS) {
        ...
        EMMC->BLOCKSIZECNT = (num << 16) | 512;
        sd_cmd(num == 1 ? CMD_READ_SINGLE : CMD_READ_MULTI,lba);
        if(sd_err) return 0;
    } else {
        EMMC->BLOCKSIZECNT = (1 << 16) | 512;
    }
    while( c < num ) {
        if(!(sd_scr[0] & SCR_SUPP_CCS)) {
...
int sd_clk(unsigned int f)
{
    ...
    int cnt = 100000;
    while((EMMC->STATUS & (SR_CMD_INHIBIT|SR_DAT_INHIBIT)) && cnt--) wait_msec(1);
    if(cnt<=0) {
        uart_puts("ERROR: timeout waiting for inhibit flag\n");
        return SD_ERROR;
    }

    EMMC->CONTROL1 &= ~C1_CLK_EN; wait_msec(10);
    x=c-1; if(!x) s=0; else {
    ...
    d=(((d&0x0ff)<<8)|h);
    EMMC->CONTROL1=(EMMC->CONTROL1&0xffff003f)|d; wait_msec(10);
    EMMC->CONTROL1 |= C1_CLK_EN; wait_msec(10);
    cnt=10000; while(!(EMMC->CONTROL1 & C1_CLK_STABLE) && cnt--) wait_msec(10);
    if(cnt<=0) {
...
int sd_init(void)
{
    ...
    /* ====GPIO_CD==== */
    /* GPIO Pin 47 takes alternate function 3 */
    r = GPIO->GPFSEL[4];
    r &= ~(7 << (7 * 3));
    GPIO->GPFSEL[4] = r;

    /* enable pin 47 pull-up control */
    GPIO->GPPUD = 2;
    wait_cycles(150); // provides the required set-up time for the control signal
    GPIO->GPPUDCLK[1] = (1 << 15); //  Assert Clock on line 15
    wait_cycles(150); // provides the required hold time for the control signal
    GPIO->GPPUD = 0; // Write to GPPUD to remove the control signal
    GPIO->GPPUDCLK[1] = 0; // Write to GPPUDCLK0/1 to remove the clock

    /* Enable high level detect for pin 47.  */
    r = GPIO->GPHEN[1]; r |= 1 << 15; GPIO->GPHEN[1] = r;

    /* ====GPIO_CLK, GPIO_CMD==== */
    /* GPIO Pin 48, 49 takes alternate function 3 */
    r = GPIO->GPFSEL[4]; r |= (7 << (8 * 3)) | (7 << (9 * 3)); GPIO->GPFSEL[4] = r;
    /* enable pin 48, 49 pull-up control */
    GPIO->GPPUD = 2; wait_cycles(150); GPIO->GPPUDCLK[1] = (1 << 16) | (1 << 17); wait_cycles(150); GPIO->GPPUD = 0; GPIO->GPPUDCLK[1] = 0;

    /* ====GPIO_DAT0, GPIO_DAT1, GPIO_DAT2, GPIO_DAT3==== */
    /* GPIO Pin 50, 51, 52, 53 takes alternate function 3 */
    r = GPIO->GPFSEL[5]; r|=(7<<(0*3)) | (7<<(1*3)) | (7<<(2*3)) | (7<<(3*3)); GPIO->GPFSEL[5] = r;
    /* enable pin 50, 51, 52, 53 pull-up control */
    GPIO->GPPUD = 2; wait_cycles(150); GPIO->GPPUDCLK[1] = (1 << 18) | (1 << 19) | (1 << 20) | (1 << 21); wait_cycles(150); GPIO->GPPUD = 0; GPIO->GPPUDCLK[1] = 0;

    sd_hv = (EMMC->SLOTISR_VER & HOST_SPEC_NUM) >> HOST_SPEC_NUM_SHIFT;
    uart_puts("EMMC: GPIO set up\n");

    /* ====Reset the card====*/
    EMMC->CONTROL0 = 0; EMMC->CONTROL1 |= C1_SRST_HC; // Enable: Reset the command handling circuit:SRST_CMD
    cnt=10000; do{wait_msec(10);} while( (EMMC->CONTROL1 & C1_SRST_HC) && cnt-- );
    if(cnt<=0) {
        uart_puts("ERROR: failed to reset EMMC\n");
        return SD_ERROR;
    }
    uart_puts("EMMC: reset OK\n");
    EMMC->CONTROL1 |= C1_CLK_INTLEN | C1_TOUNIT_MAX; // Clock enable for internal EMMC clocks for power saving:CLK_INTLEN. Data timeout unit exponent
    wait_msec(10);
