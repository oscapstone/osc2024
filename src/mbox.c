...
void get_arm_base_memory_sz() {
    ...
    if (mbox_call(MBOX_CH_PROP)) {
        uart1_printf("memory base address: 0x%x\n", mbox[6]);
        uart1_printf("memory size: %d bytes\n", mbox[5]);
    } else {
        uart1_printf("Unable to query arm memory and size..\n");
    }
}
...
void get_board_serial() {
    ...
    if (mbox_call(MBOX_CH_PROP)) {
        uart1_printf("Serial number: 0x%x%x\n", mbox[6], mbox[5]);
    } else {
        uart1_printf("Unable to query serial number..\n");
    }
}
...
void get_board_revision() {
    ...
    if (mbox_call(MBOX_CH_PROP)) {
        uart1_printf("Board revision number: 0x%x\n", mbox[5]); // it should be 0xa020d3 for rpi3 b+
    } else {
        uart1_printf("Unable to query board revision..\n");
    }
}
