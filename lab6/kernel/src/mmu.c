...
void mmu_memfail_abort_handle(esr_el1_t* esr_el1)
{
        ...
        (esr_el1->iss & 0x3f) == TF_LEVEL3)
    {
        ...
        uart_sendline("the_area_ptr->virt_addr: 0x%x, the_area_ptr->phys_addr: 0x%x\r\n", the_area_ptr->virt_addr, the_area_ptr->phys_addr);
        uart_sendline("addr_offset: 0x%x\r\n", addr_offset);
        map_one_page(
            PHYS_TO_VIRT(curr_thread->context.pgd), 
            the_area_ptr->virt_addr + addr_offset, 
            the_area_ptr->phys_addr + addr_offset, 
            flag
        );
    }
    else
