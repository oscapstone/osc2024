...
void uart_send ( char c )
{
	if (c == '\n')
		uart_send('\r');
	while(1) {
		if(get32(PA2VA(AUX_MU_LSR_REG))&0x20) 
			break;
	}
...
char uart_recv ( void )
{
	...
	return(get32(PA2VA(AUX_MU_IO_REG)));
}
...
void irq_uart_rx_exception() {
	if((uart_read_index + 1) % BUFFER_SIZE == uart_read_head) {
		...
		unsigned int ier = get32(PA2VA(AUX_MU_IER_REG));
		// only enable receiver interrupt for now
		ier &= ~0x01;
		put32(PA2VA(AUX_MU_IER_REG), ier);
		return;
	}
	char c = get32(PA2VA(AUX_MU_IO_REG))&0xFF;
...
void irq_uart_tx_exception() {
	if (uart_write_index != uart_write_head) {
		put32(PA2VA(AUX_MU_IO_REG), uart_write_buffer[uart_write_index++]);
...
void uart_irq_handler() {
	unsigned int iir = get32(PA2VA(AUX_MU_IIR_REG));
	unsigned int ier = get32(PA2VA(AUX_MU_IER_REG));
	ier &= ~0x02;
	ier &= ~0x01;
	put32(PA2VA(AUX_MU_IER_REG), ier);
...
void uart_enable_interrupt( void ) {
	...
	unsigned int enable_irqs_1 = get32(PA2VA(ENABLE_IRQS_1));
	enable_irqs_1 |= 0x01 << 29;
	put32(PA2VA(ENABLE_IRQS_1), enable_irqs_1);
}
