...
void initrd_list()
{
    ...
    while (!memcmp(buf, CPIO_HEADER_MAGIC, 6))
    {
        ...
        uart_hex(hex2bin(header->c_mode, sizeof(header->c_mode))); // mode (access rights + type)
        uart_send(' ');
        ...
        *data = (void *)align_up(((unsigned long)buf)
            + sizeof(struct cpio_newc_header) + filename_length, CPIO_ALIGNMENT);
        *next = (struct cpio_newc_header *)align_up(((unsigned long)*data) + filesize, CPIO_ALIGNMENT);
        buf += (sizeof(struct cpio_newc_header) + ns + fs + 3) & ~3;
        uart_puts(buf);
        uart_puts("\n");
    }
}
