...
if platform == "linux" or platform == "linux2":
    ...
    with open(args.filename,'rb') as fd:
        with Serial(args.device, args.baud) as ser:
            ...
            kernel_raw = fd.read() #img中的內容儲存到kernel_raw
            length = len(kernel_raw) #紀錄長度

            print("Kernel image size : ", hex(length))
            for i in range(8):
                ser.write(p64(length)[i:i+1]) #把文件長度轉為8字節的字串，逐字節寫入
                ser.flush() #確保每次寫入的數據立即發送
            ...
            for i in range(length):
                ...
                ser.write(kernel_raw[i: i+1]) #逐字節發送img檔案的內容
                ser.flush()
                if i % 100 == 0: #每發送100字節打印一次進度
                    print("{:>6}/{:>6} bytes".format(i, length))

            print("{:>6}/{:>6} bytes".format(length, length)) #印出最後傳輸的字節內容
