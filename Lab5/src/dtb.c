...
// memory reservation block - https://devicetree-specification.readthedocs.io/en/latest/chapter5-flattened-format.html#sect-fdt-memory-reservation-block
...
void dtb_cb_init(uint32_t type, char *name, void *value, uint32_t name_size){
    ...
    {
        CPIO_START = (void *)(unsigned long long)uint32_endian_big2lttle(*(uint32_t*)value);
    }
    if (type == FDT_PROP && strcmp(name, "linux,initrd-end") == 0)
    {
        CPIO_END = (void *)(unsigned long long)uint32_endian_big2lttle(*(uint32_t *)value);
    }
