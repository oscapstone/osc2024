...
unsigned int str2Int(char* str, int len) {
    ...
    for (int i = 0; i < len; i++) {
        switch (str[i]) {
            case 'A':
                tot += 10 * power(10, len - i - 1);
                break;
            case 'B':
                tot += 11 * power(10, len - i - 1);
                break;
            case 'C':
                tot += 12 * power(10, len - i - 1);
                break;
            case 'D':
                tot += 13 * power(10, len - i - 1);
                break;
            case 'E':
                tot += 14 * power(10, len - i - 1);
                break;
            case 'F':
                tot += 15 * power(10, len - i - 1);
                break;
            default:
                // Ascii code for '0' is 48.
                tot += (str[i] - 48) * power(10, len - i - 1);
        }
    }
