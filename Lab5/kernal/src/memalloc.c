...
int buddy_init(){
    ...
    memory_reserve(0x80000,&_stack_top); //kernel
    memory_reserve(_dtb_addr,dtb_end()); //device tree
    memory_reserve(cpio_addr,cpio_end()); //cpio
...
void* fr_malloc(unsigned int size){
    ...
    if(allocated_frame == NULL){
        ...
        puts("\r\n");
        return allocated_frame->address;
    }
