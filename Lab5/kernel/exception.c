...
// System call table.
static uint64_t (*syscall_table[])(uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3) = {
    [SYS_GETPID]        =   (void *)getpid,
    [SYS_UART_READ]     =   (void *)uart_recv,
    [SYS_UART_WRITE]    =   (void *)uart_send
};
...
void handle_sync_el0_64(trap_frame_t* tf) {
    ...
    // Retrieve system call number from user process. Typically stored in x8 in arm.
    int syscall = tf->x8;

    // Check whether the system call is valid.
    if (syscall < sizeof(syscall_table) / sizeof(syscall_table[0]) && syscall_table[syscall]) {
        // Send the arguments if required.
        tf->x0 = syscall_table[syscall](tf->x0, tf->x1, tf->x2, tf->x3);
