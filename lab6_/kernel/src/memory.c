...
void init_allocator()
{
    ...
    */
    ...
    dtb_find_and_store_reserved_memory(); // find spin tables in dtb                                                          // find spin tables in dtb
    memory_reserve(PHYS_TO_VIRT(MMU_PGD_ADDR), PHYS_TO_VIRT(MMU_PTE_ADDR+0x2000), "Page table"); // // PGD's page frame at 0x1000 // PUD's page frame at 0x2000 PMD 0x3000-0x5000
    memory_reserve((unsigned long long)&_kernel_start, (unsigned long long)&_kernel_end, "kernel code & data");                         // kernel code and data
    memory_reserve((unsigned long long)CPIO_DEFAULT_START, (unsigned long long)CPIO_DEFAULT_END, "CPIO");
}
...
void* page_malloc(unsigned int size)
{
    ...
    for (int i = FRAME_IDX_0; i <= FRAME_IDX_FINAL; i++){
        if (size <= (PAGESIZE << i)) {
            val = i;
            memory_sendline("        block size = 0x%x, val = %d\n", PAGESIZE << i, i);
            break;
        }
...
void dump_page_info(char *msg){
    unsigned int exp2 = 1;
    memory_sendline("        ------------- [ {%s} Number of Available Page Blocks  ] -------------\r\n        | ", msg);
    for (int i = FRAME_IDX_0; i <= FRAME_IDX_FINAL; i++) {
...
{
    unsigned int exp2 = 1;
    memory_sendline("    ---------- [ {%s} Number of Available Cache Blocks ] --------- \r\n    | ", msg);
    for (int i = CACHE_IDX_0; i <= CACHE_IDX_FINAL; i++){
...
void memory_reserve(unsigned long long start, unsigned long long end, char *name)
{
    ...
    memory_sendline("0x%x ~ ", start);
    memory_sendline("0x%x\r\n",end);
    ...
    for (; order >= 0; order--) {
        ...
        list_for_each(pos, &frame_freelist[order]) {
            ...
            */
                ...
                memory_sendline("    [!] partial intersection in 0x%x - 0x%x\n", pagestart, pageend);
                dump_page_info("Before");
                list_del_entry(pos);
