...
void uart_init()
{
    ...
    *GPPUD = 0;   //  P101 top. 00- = off - disable pull-up/down 
    ...
    r=150; while(r--) { asm volatile("nop"); } //asm:行內組譯 volatile:每次需要儲存或讀取這個變數的時候，都會直接從變數位址中讀取資料 => 不最佳化
    ...
    *GPPUDCLK0 = (1<<14)|(1<<15); //0110 0000 0000 0000
   ...
   //AUX_MU_BAUD access to baudrate counter
   //baudrate: bit/per sec
    *AUX_MU_IIR = 0xc6; // 0xc6 = 11000110
   // bit 6 bit 7 No FIFO. Sacrifice reliability(buffer) to get low latency    
   // 2:1
   // Writing with bit 1 set will clear the receive FIFO
   // Writing with bit 2 set will clear the transmit FIFO
   // 5:3 always read as zero
   // 7:6
    ...
    *AUX_MU_CNTL = 3; // enable Transmitter,Receiver
    ...
    // Since each GPFSEL controls 10 pin, GPFSEL1 controls 10-19 (14/15)
    // That's why I need GPFSEL1  
    r=*GPFSEL1;
    r&=~((7<<12)|(7<<15)); // gpio14, gpio15 clear to 0  7 = 111 (7<<12) = 111.0000.0000.0000 (7<<15) = 1000.0000.0000.0000 or => 1111.0000.0000.0000
                            //  not => 0000.1111.1111.1111 => keep 11:0 bit
    r|=(2<<12)|(2<<15);    // set gpio14 and 15 to 010/010 which is alt5  2 = 10 (2<<12) = 10.0000.0000.0000.0000 (2<<15) = 0100.0000.0000.0000.0000
                            // or => 0110.0000.0000.0000.0000 => set 15/14 to 1
...
void uart_send_char(unsigned int c) {
    ...
    do{asm volatile("nop");}while(!(*AUX_MU_LSR&0x20)); //if AUX_MU_LSR bit 5 not 1 then nop
...
char uart_get_char() {
    ...
    do{asm volatile("nop");}while(!(*AUX_MU_LSR&0x01)); //if AUX_MU_LSR bit 0 not 1 then nop
    /* read it and return */
    r=(char)(*AUX_MU_IO);
 ...
 */
void uart_binary_to_hex_no0x(unsigned int d) {
    unsigned int n;
    int c;
    for(c=28;c>=0;c-=4) { //28 24 20 16 12 8 4 0
        // get highest tetrad
