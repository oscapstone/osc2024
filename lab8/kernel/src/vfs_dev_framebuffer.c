...
struct file_operations dev_framebuffer_operations = {
    dev_framebuffer_write, (void *)dev_framebuffer_op_deny, dev_framebuffer_open,
    dev_framebuffer_close, dev_framebuffer_lseek64, (void *)dev_framebuffer_op_deny};
...
int init_dev_framebuffer()
{
    ...
    if (mbox_call(MBOX_TAGS_ARM_TO_VC, (unsigned int)((unsigned long)&pt)) && pt[20] == 32 && pt[28] != 0)
    {
        ...
        lfb = (void *)PHYS_TO_KERNEL_VIRT(((unsigned long)pt[28])); // raw frame buffer address
    }
    else
    {
...
int dev_framebuffer_write(struct file *file, const void *buf, size_t len)
{
    ...
    {
        ERROR("How come? dev_framebuffer_write to no where!\r\n");
