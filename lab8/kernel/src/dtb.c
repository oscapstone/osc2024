...
void dtb_callback_initramfs(uint32_t node_type, char *name, void *value, uint32_t name_size)
{
	...
	{
		CPIO_DEFAULT_START = (void *)(unsigned long long)PHYS_TO_KERNEL_VIRT(uint32_endian_big2little(*(uint32_t *)value));
	}
	if (node_type == FDT_PROP && strcmp(name, "linux,initrd-end") == 0)
	{
		CPIO_DEFAULT_END = (void *)(unsigned long long)PHYS_TO_KERNEL_VIRT(uint32_endian_big2little(*(uint32_t *)value));
	}
}
...
void dtb_find_and_store_reserved_memory()
{
	...
	char *dt_mem_rsvmap_ptr = (char *)((char *)header + uint32_endian_big2little(header->off_mem_rsvmap));
	...
	{
		unsigned long long start = PHYS_TO_KERNEL_VIRT(uint64_endian_big2little(reverse_entry->address));
		unsigned long long end = uint64_endian_big2little(reverse_entry->size) + start;
		memory_reserve(start, end);
		reverse_entry++;
	}

	// reserve device tree itself
	memory_reserve((unsigned long long)dtb_ptr, (unsigned long long)dtb_ptr + uint32_endian_big2little(header->totalsize));
}