...
int cmd_help(){
    ...
    puts("thread:");for(int i=6;i<32;i++)puts(" ");puts("test thread.\r\n");
...
int cmd_info()
{
    ...
    if (mbox_call_buf(MBOX_TAGS_ARM_TO_VC, ((unsigned long)&pt)))
    {
...
int cmd_test(){
    unsigned long doge;
    unsigned long long cheems;
    puts("size of unsigned long:");
    put_int(sizeof(doge));
    puts("\r\n");
    puts("size of unsigned long long:");
    put_int(sizeof(cheems));
    puts("\r\n");
    // int* outrange=0xffff000050000000;
    // put_int(*outrange);
    // puts("\r\n");
    //asm("msr ttbr0_el1, xzr");
    //Identity_Paging_el0();
    puts("syscall.img size:");
    put_int(cpio_size("syscall.img"));
    puts("\r\n");
    void* proc_start=fr_malloc(cpio_size("syscall.img"));
    cpio_load("syscall.img",proc_start);
    return 0;
}

int cmd_dy_malloc(){
    char* doge=fr_malloc(0x1000);
    doge="fuckyou\r\n";
    puts(doge);
    puts("after fr_malloc\r\n");
    int* arr[100];
    for(int i=0;i<100;i++){
...
int cmd_handler(char* cmd){
    ...
    else if(!strcmp(cmd,"thread"))cmd_thread_test();
