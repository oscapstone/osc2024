    ...
    while (cur_ptr < end_ptr) {
        ...
        switch (token) {
            case FDT_BEGIN_NODE:
                ...
                cur_ptr = memory_padding(name_ptr + str_len(name_ptr) + 1, 2);  // the string length should contain '\0'
                break;
            case FDT_PROP:
                ...
                cur_ptr = memory_padding(data_ptr + data_len, 2);
                break;
            case FDT_END_NODE:
            case FDT_NOP:
                cb(token, 0, 0, 0);
...
static byteptr_t _dtb_end = nullptr;
byteptr_t   dtb_get_ptr() { return _dtb_ptr; }
byteptr_t   dtb_get_end() { return _dtb_end; }


uint32_t        
dtb_set_ptr(byteptr_t ptr) 
{ 
    fdt_header_ptr_t header = (fdt_header_ptr_t) ptr;
    if (_dtb_get_uint32((byteptr_t) &(header->magic)) != 0xd00dfeed) {
        return FDT_TRAVERSE_HEADER_ERROR;
    } 
    _dtb_ptr = ptr;
    _dtb_end = (byteptr_t) ((uint64_t) dtb_get_ptr() + _dtb_get_uint32((byteptr_t) &(header->totalsize)));
    return FDT_TRAVERSE_CORRECT;
}
...
fdt_find_initrd_addr(uint32_t token, const byteptr_t name_ptr, const byteptr_t data_ptr, uint32_t v)
{
    ...
    else if (token == FDT_PROP && str_eql(name_ptr, "linux,initrd-end")) {
        byteptr_t _addr = (byteptr_t) (0l | _dtb_get_uint32(data_ptr));
        initrd_set_end(_addr);
    }
}
